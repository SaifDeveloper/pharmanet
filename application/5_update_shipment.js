...
async function main(nameOfOrg, buyerCRN, drugName, transporterCRN) {
  try {
    ...
    const updatedShipmentBuffer = await contract.submitTransaction(
      "updateShipment",
      buyerCRN,
      drugName,
      transporterCRN
    );
    console.log("updatedShipmentBuffer: ",updatedShipmentBuffer);
    console.log('Transaction has been submitted');
    let updatedShipment = JSON.parse(updatedShipmentBuffer.toString());

    let response = JSON.parse(JSON.stringify(updatedShipment).replace(/\\u0000/g,''));
