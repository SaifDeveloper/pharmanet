...
async function main(nameOfOrg, companyCRN, companyName, location, organisationRole) {
  try {
    ...
    let registeredCompanyBuffer = await contract.submitTransaction('registerCompany', companyCRN , companyName, location, organisationRole);
    console.log('Transaction has been submitted');

    let registeredCompany = JSON.parse(registeredCompanyBuffer.toString());
    
    let response = JSON.parse(JSON.stringify(registeredCompany).replace(/\\u0000/g,''));
