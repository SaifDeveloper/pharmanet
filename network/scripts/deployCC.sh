...
CC_END_POLICY=${8:-"NA"}
CC_COLL_CONFIG=${9:-"NA"}
# "OR (ManufacturerMSP.peer,DistributorMSP.peer,RetailerMSP.peer,ConsumerMSP,TransporterMSP)"
DELAY=${10:-"3"}
MAX_RETRY=${11:-"5"}
VERBOSE=${12:-"false"}
...
if [ "$CC_END_POLICY" = "NA" ]; then
  # CC_END_POLICY=""
  CC_END_POLICY="OR (ManufacturerMSP.peer,DistributorMSP.peer,RetailerMSP.peer,ConsumerMSP,TransporterMSP)"
else
...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma.net --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma.net --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma.net --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
installChaincode 5 0

infoln "Installing chaincode on peer1.manufacturer..."
