...
function createOrgs() {
  ...
  infoln "Generating CCP files for Manufacturer and Distributor"
...
function networkDown() {
  # stop org3 containers also in addition to manufacturer and distributor, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/transporter/msp organizations/fabric-ca/transporter/tls-cert.pem organizations/fabric-ca/transporter/ca-cert.pem organizations/fabric-ca/transporter/IssuerPublicKey organizations/fabric-ca/transporter/IssuerRevocationPublicKey organizations/fabric-ca/transporter/fabric-ca-server.db'
