...
class PharmanetContract extends Contract {
    ...
    async registerCompany(
        ...
        if ("ConsumerMSP" !== mspID) {
            ...
            if (
                ...
                let companyDataBuffer = Buffer.from(
    ...
    async addDrug(ctx, drugName, serialNo, mfgDate, expDate, companyCRN) {
        ...
        while (!manufacturerFound) {
            ...
            if (
                ...
                throw new Error("Invalid companyCRN");
            }
            ...
            const generateKeyDrugOwner = await ctx.stub.createCompositeKey(
                "org.pharma-network.pharmanet.company",
                [returnedCompanyCRN, returnedCompanyName]
            );
            ...
            if ("ManufacturerMSP" === mspID) {
                const productID = ctx.stub.createCompositeKey(
                    "org.pharma-network.pharmanet.drug",
                    [drugName, serialNo]
                );
                ...
                throw new Error("Only Manufacturer can add drug!");
            }
        }
    }
    ...
    async createPO(ctx, buyerCRN, sellerCRN, drugName, quantity) {
        ...
        let indexName = "org.pharma-network.pharmanet.company";
        ...
        if ("RetailerMSP" !== mspID && "DistributorMSP" !== mspID) {
            // return "Only Distributor and Retailer can create a purchase request!";
            throw new Error("Only Distributor and Retailer can create a purchase request!")
        } else {
            // Go ahead and check the hierarchy
            let sellerCRNResultsIterator =
                await ctx.stub.getStateByPartialCompositeKey(indexName, [
                    sellerCRN,
                ]);

            var sellerCRNFound = false;
            while (!sellerCRNFound) {
                let sellerCRNResponseRange =
                    await sellerCRNResultsIterator.next();

                if (
                    !sellerCRNResponseRange ||
                    !sellerCRNResponseRange ||
                    !sellerCRNResponseRange.value.key
                ) {
                    ...
                    ({ objectType, attributes } =
                        await ctx.stub.splitCompositeKey(
                            sellerCRNResponseRange.value.key
                        ));
                    ...
                    var generateSellerCompanyID =
                        await ctx.stub.createCompositeKey(indexName, [
                            returnedSellerCompanyName,
                            returnedSellerCompanyCRN,
                        ]);

                    var sellerCompanyBuffer = await ctx.stub
                        .getState(generateSellerCompanyID)
                        .catch((err) => console.log(err));
                    console.log(
                        "Seller Company Details are=> " +
                            sellerCompanyBuffer.toString()
                    );
                }
            }

            let buyerCRNResultsIterator =
                await ctx.stub.getStateByPartialCompositeKey(indexName, [
                    buyerCRN,
                ]);
            ...
            while (!buyerCRNFound) {
                let buyerCRNResponseRange =
                    await buyerCRNResultsIterator.next();

                if (
                    !buyerCRNResponseRange ||
                    !buyerCRNResponseRange ||
                    !buyerCRNResponseRange.value.key
                ) {
                    return "Invalid Seller CompanyCRN";
                } else {
                    buyerCRNFound = true;
                    ...
                    var generateBuyerCompanyID =
                        await ctx.stub.createCompositeKey(indexName, [
                            returnedBuyerCompanyName,
                            returnedBuyerCompanyCRN,
                        ]);

                    var buyerCompanyBuffer = await ctx.stub
                        .getState(generateBuyerCompanyID)
                        .catch((err) => console.log(err));
                    console.log(
                        "Buyer Company Details are=> " +
                            buyerCompanyBuffer.toString()
                    );
                }
            }
        }
        ...
        if (buyerData.organisationRole === "Retailer") {
            ...
            if (sellerData.organisationRole === "Distributor") {
                ...
                const poID = ctx.stub.createCompositeKey(
                ...
                console.log(
                    "purchaseOrderObject created is==> " + purchaseOrderObject
                );

                let purchaseOrderDataBuffer = Buffer.from(
                    JSON.stringify(purchaseOrderObject)
                );
                ...
                let returnValue =
                    "Sorry!" +
                    buyerData.organisationRole +
                    " can't purchase from " +
                    sellerData.organisationRole;
                console.log(
                    "Sorry!" +
                        buyerData.organisationRole +
                        " can't purchase from " +
  ...
  async createShipment(ctx, buyerCRN, drugName, listOfAssets, transporterCRN) {
    ...
    async updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
        ...
        let transporterResultsIterator =
            await ctx.stub.getStateByPartialCompositeKey(
                "org.pharma-network.pharmanet.company",
                [transporterCRN]
            );
        ...
        if (
            !transporterResponseRange ||
            !transporterResponseRange ||
            !transporterResponseRange.value.key
        ) {
            return "Invalid transporterCRN";
        }
        console.log(
            "ResponseRange.value.key=>" + transporterResponseRange.value.key
        );
        ...
        var generateTransporterForShipmentUpdation =
            await ctx.stub.createCompositeKey(
                "org.pharma-network.pharmanet.company",
                [transportForUpdateShipmentCRN, transportForUpdateShipmentName]
            );

        console.log(
            "this is the generated transporter composite key=>" +
                generateTransporterForShipmentUpdation
        );
        ...
        if (transportForUpdateShipmentCRN === transporterCRN) {
            ...
            let generatedShipmentCompositeKey =
                await ctx.stub.createCompositeKey(
                    "org.pharma-network.pharmanet.shipment",
                    [buyerCRN, drugName]
                );

            let shipmentDataBuffer = await ctx.stub
                .getState(generatedShipmentCompositeKey)
                .catch((err) => console.log(err));
            console.log("This is the shipment details" + shipmentDataBuffer);
            let parsedShipmentData = JSON.parse(shipmentDataBuffer.toString());
            console.log(
                "transporter composite key what sin shipment=> " +
                    parsedShipmentData.transporter
            );
            console.log(
                "generated transporter=>" +
                    generateTransporterForShipmentUpdation
            );
            if (
                parsedShipmentData.transporter ===
                ...
                //Once you have updated the owner of the drug put the state back to the drug
                let statusChangeShipmentBuffer = Buffer.from(
                    JSON.stringify(parsedShipmentData)
                );
                //Wait untill you get the successfully delivered response, then display. Else the status will be in still "in-transit"
                await ctx.stub.putState(
                    generatedShipmentCompositeKey,
                    statusChangeShipmentBuffer
                );
                ...
                //Buyer compositeKey
                let buyerCRNResultsIteratorForOwner =
                    await ctx.stub.getStateByPartialCompositeKey(
                        "org.pharma-network.pharmanet.company",
                        [buyerCRN]
                    );
                ...
                while (!buyerCRNFoundForOwner) {
                    let buyerCRNFoundForOwnerResponseRange =
                        await buyerCRNResultsIteratorForOwner.next();
                    ...
                    if (
                        ...
                        console.log(
                            "returnedBuyerCompanyNameForOwner=> " +
                                returnedBuyerCompanyNameForOwner
                        );
                        console.log(
                            "returnedBuyerCompanyCRNForOwner=> " +
                                returnedBuyerCompanyCRNForOwner
                        );

                        var generateBuyerCompanyIDForOwner =
                            await ctx.stub.createCompositeKey(
                                "org.pharma-network.pharmanet.company",
                                [
                                    returnedBuyerCompanyCRNForOwner,
                        ...
                        console.log(
                            "Transporter composite key created=> " +
                                generateBuyerCompanyIDForOwner
                        );
                    }
                }
                //Iterate through the drug list in the shipment
                ...
                for (let i = 0; i <= drugsInShipment.length - 1; i++) {
                    ...
                    console.log(
                        "drugCompositeKeyID is=> " + drugCompositeKeyID
                    );
                    let drugDataFromAddDrugBuffer = await ctx.stub
                        .getState(drugCompositeKeyID)
                        .catch((err) => console.log(err));
                    let JSONDrugDetailsForUpdation = JSON.parse(
                        drugDataFromAddDrugBuffer.toString()
                    );

                    console.log(
                        "JSONDrugDetailsForUpdation=>" +
                            JSONDrugDetailsForUpdation
                    );
                    console.log(
                        "The shipment field for " +
                            drugCompositeKeyID +
                            " is " +
                            JSONDrugDetailsForUpdation.shipment
                    );
                    console.log(
                        "The owner field for " +
                            drugCompositeKeyID +
                            " is " +
                            JSONDrugDetailsForUpdation.owner
                    );
                    ...
                    //Once you have updated the owner of the drug put the state back to the drug
                    let modifiedDrugDetailsObject = Buffer.from(
                        JSON.stringify(JSONDrugDetailsForUpdation)
                    );
                    await ctx.stub.putState(
                        drugCompositeKeyID,
                        modifiedDrugDetailsObject
                    );
                    // return modifiedDrugDetailsObject;
                    JSON.parse(modifiedDrugDetailsObject.toString())
                }
            }

            console.log("updatedShipment==========>", JSON.parse(shipmentDataBuffer.toString()))
    ...
    async retailDrug(ctx, drugName, serialNo, retailerCRN, customerAadhar) {
        ...
        const drugCompositeKeyForSearch = ctx.stub.createCompositeKey(
            "org.pharma-network.pharmanet.drug",
            [drugName, serialNo]
        );
        let drugCompositeKeyForSearchBuffer = await ctx.stub
            ...
            .catch((err) => console.log(err));
        let JSONDrugCompositeKeyForSearch = JSON.parse(
            drugCompositeKeyForSearchBuffer.toString()
        );

        //Find retailers composite key
        console.log(
            "JSONDrugCompositeKeyForSearch.owner=> " +
                JSONDrugCompositeKeyForSearch.owner
        );
        console.log("retailerCRN=> " + retailerCRN);

        //Buyer compositeKey
        let retailerCRNResultsIteratorForComparision =
            await ctx.stub.getStateByPartialCompositeKey(
                "org.pharma-network.pharmanet.company",
                [retailerCRN]
            );
        ...
        while (!retailerCRNFoundForComparision) {
            let retailerCRNResultsIteratorForComparisionResponseRange =
                await retailerCRNResultsIteratorForComparision.next();
            ...
            if (
                ...
                let attributes;
                ({ objectType, attributes } = await ctx.stub.splitCompositeKey(
                    retailerCRNResultsIteratorForComparisionResponseRange.value
                        .key
                ));
                ...
                console.log(
                    "returnedBuyerCompanyNameForOwner=> " +
                        returnedRetailerCompanyNameForComparision
                );
                console.log(
                    "returnedRetailerCompanyCRNForComparision=> " +
                        returnedRetailerCompanyCRNForComparision
                );

                var generateRetailerCompanyIDForOwner =
                    await ctx.stub.createCompositeKey(
                        "org.pharma-network.pharmanet.company",
                        [
                            returnedRetailerCompanyCRNForComparision,
        ...
        if (
            JSONDrugCompositeKeyForSearch.owner ===
            generateRetailerCompanyIDForOwner
        ) {
            ...
            //Once you have updated the owner of the drug put the state back to the drug
            let modifiedDrugDetailsObjectForCustomer = Buffer.from(
                JSON.stringify(JSONDrugCompositeKeyForSearch)
            );
            await ctx.stub.putState(
                drugCompositeKeyForSearch,
                modifiedDrugDetailsObjectForCustomer
            );
    ...
    async viewDrugCurrentState(ctx, drugName, serialNo) {
        ...
        let drugDataBuffer = await ctx.stub
            .getState(productID)
            .catch((err) => console.log(err));
